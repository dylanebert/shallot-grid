...
- `src/` is the plugin and its shader. `examples/world-grid` is the one example; its harness imports the checked-out source so the browser test cannot resolve a published Grid, while a shipped consumer uses the package root and Shallot's declared subpaths.
